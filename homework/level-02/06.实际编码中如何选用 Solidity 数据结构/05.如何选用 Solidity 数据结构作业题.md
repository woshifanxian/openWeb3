...
减少存储变量的写操作。
优先使用内存或临时数据处理，尽量少写入 Storage。
将多个变量存储到一个结构体或压缩为更小的类型。
例如，将多个布尔值合并到一个 uint256 中：
使用合适的数据类型，例如 uint8 而不是 uint256，以减少存储空间。
尽量将多个小数据类型打包到一个存储槽（Solidity 会自动优化布局）。
动态数组和映射需要额外的 Gas 开销。
如果数组或映射中的数据不需要频繁修改，尽量减少使用。
在合约中避免在循环中直接操作存储或消耗大量 Gas 的函数。
优化建议：使用内存数组或外部处理批量操作。
如果逻辑简单，优先内联代码，减少函数间的 Gas 消耗。
仅记录必要的信息，避免额外 Gas 消耗。
